...
static int user_property_set_event_handler(const int devid, const char *request, const int request_len)
{
    ...
    if (!request) {
       ...
       HeatSwitch = cJSON_GetObjectItem(root, "HeatSwitch");
       if (HeatSwitch) {
    	   set_value.HeatSwitch=HeatSwitch->valueint;
    	   Heater_set_on(HeatSwitch->valueint);
       }

       HeatSV = cJSON_GetObjectItem(root, "TargetTemperature");
       if (HeatSV) {
    	   set_value.HeatSV=HeatSV->valuedouble;
    	   Heater_set_sv(HeatSV->valuedouble);
       }

       FanSwitch = cJSON_GetObjectItem(root, "FanSwitch");
       if (FanSwitch) {
    	   set_value.FanSwitch=FanSwitch->valueint;
    	   FAN_set_speed(set_value.FanSwitch,set_value.FanSpeed);
       }

       FanSpeed = cJSON_GetObjectItem(root, "FANspeed");
       if (FanSpeed) {
    	   set_value.FanSpeed=FanSpeed->valueint;
    	   FAN_set_speed(set_value.FanSwitch,set_value.FanSpeed);
       }
...
static void user_post_property(char *payload)
{
    static int cnt = 0;
    int res = 0;

    //char property_payload[30] = {0};
    HAL_Snprintf(payload, sizeof(payload), "{\"Counter\": %d}", cnt++);
